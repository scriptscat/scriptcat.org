...
![网络规则列表](v1.5.assets/network-rules-list.png)

编辑抽屉提供常用场景模板与自定义两种入口，Cookie、Authorization、Host、Origin 等会泄露或伪造调用者身份的
请求头一律不允许改写；「所有网站」范围属于高危操作，保存前会再次确认。填好规则后可以直接用「试一试」输入一个
网址，看看它会被怎么处理。

![编辑网络规则](v1.5.assets/network-rules-edit.png)

## 💥 外部接入（MCP 桥接）[#1573](https://github.com/scriptscat/scriptcat/pull/1573)

通过本地 `sctl` 守护进程，CLI 与 MCP 客户端可统一接入脚本猫：列出/读取脚本元数据、安装、启用/停用、删除、编辑等操作均经**分级授权与人工确认页面**把关，可按「拒绝 / 允许 / 本会话允许」三档控制，所有操作留存审计记录。
