...
現在 Tampermonkey を使用しており、ScriptCat に移行したい場合は、スムーズな移行のための手順とヒントをご紹介します。

## Tampermonkey からバックアップをエクスポート

まず、Tampermonkey アイコンをクリックしてダッシュボードに入ります。

![image-20250716225507707](./migrate-from-tampermonkey.assets/image-20250716225507707.webp)

`ユーティリティ` をクリックし、zip ファイルセクションの下の `エクスポート` をクリックして zip ファイルをエクスポートします。

![image-20250716225611932](./migrate-from-tampermonkey.assets/image-20250716225611932.webp)

## ScriptCat にインポート

ScriptCat 拡張機能で、ダッシュボードアイコンをクリックして管理パネルに入ります。
...
その後、新しく開いたページでインポートするスクリプトを選択または全選択し、`インポート` ボタンをクリックします。
