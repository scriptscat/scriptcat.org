...
* Кросс-доменный HTTP-запрос, который может обходить CSP, с поддержкой доменов, объявленных через `@connect`. Часть функциональности отсутствует; функция cookie сейчас не поддерживается в Firefox. Для нормального доступа требуется авторизация пользователя; хост, описанный `@connect`, может пропускать авторизацию пользователя.
...
Нужно объявить оперируемый хост через `@connect`, и для использования требуется авторизация пользователя. Хотя операция `GM_cookie.list` совместима с Tampermonkey, это не рекомендуется ради согласованности.
