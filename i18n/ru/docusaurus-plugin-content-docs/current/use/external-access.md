...
:::info Соединение остаётся локальным
sctl слушает только `127.0.0.1` и недоступен из локальной сети или интернета. Расширение подключается к демону,
стороны создают долговременный ключ с помощью одноразового кода, а последующие соединения используют взаимную
аутентификацию.
:::
