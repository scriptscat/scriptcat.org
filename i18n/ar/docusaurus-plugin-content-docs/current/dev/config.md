...
المحتوى داخل `==UserConfig==` يأتي بعد `==UserScript==`، ويصف بعض الخيارات القابلة للتكوين من قبل المستخدم للسكرپت. التكوين مكتوب بصيغة [YAML](https://yaml.org/):
...
بمجرد تعريفه هنا، يظهر زر تكوين في لوحة التحكم ليتمكن المستخدم من التكوين. يستخدم المطورون `GM_getValue` لقراءة قيمة التكوين، مع التعبير عن المفتاح بصيغة `group.config`.
