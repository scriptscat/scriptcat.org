---
title: 從 Tampermonkey 遷移到 ScriptCat
---

如果您目前使用 Tampermonkey 並想遷移到 ScriptCat，以下是一些步驟和技巧，幫助您順利完成遷移。

## 從 Tampermonkey 匯出備份

首先，點擊 Tampermonkey 圖示進入控制面板。

![image-20250716225507707](./migrate-from-tampermonkey.assets/image-20250716225507707.webp)

點擊 `實用工具`，然後在 zip 檔案區塊下點擊 `匯出` 以匯出 zip 檔案。
...
選擇 `工具`，點擊 `匯入檔案`，選擇先前匯出的 Tampermonkey zip 檔案，然後點擊 `開啟` 進行匯入。
